    ...
    public ActionForward execute(ActionMapping map, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equals("Ingresar")) {
            ...
            if (contra == null || contra.equals("")) {
                adver2 = "* La Contraseña es Obligatorio <br>";
            }
            if (!adver.equals("")) {
                formBean.setError("<div class='alert alert-danger'>" + adver1+"<div>" +adver2+ "<div>");
                return map.findForward(error);
            } else {
                MantenimientoUsuario mantUser = new MantenimientoUsuario();
                Usuario user = mantUser.entrar(correo, contra);
                if (user == null) {
                    formBean.setError("<div class='alert alert-danger'>Usurio y/o contraseña son incorrectas</div>"+ adver1+"<div>"+adver2);
                    return map.findForward(error);
                } else {
